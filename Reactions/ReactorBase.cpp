...
ReactorBase::set_typ_vals_ode(const std::vector<amrex::Real>& ExtTypVals)
{
  ...
  m_typ_vals_gpu = (amrex::Real*)amrex::The_Arena()->alloc(size_ETV * sizeof(amrex::Real));
  amrex::Gpu::copy(amrex::Gpu::hostToDevice, &(m_typ_vals[0]), &(m_typ_vals[0])+size_ETV, m_typ_vals_gpu);
#endif
}
...
#if defined(AMREX_USE_HIP) || defined(AMREX_USE_CUDA)
    auto nbThreads = 256;
    auto nbBlocks = std::max(1, neq_tot / nbThreads);
